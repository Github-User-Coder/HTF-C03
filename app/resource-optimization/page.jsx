    ...
    <div className="container mx-auto p-6">
      ...
      {optimizationResults ? (
          ...
          <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <div className="bg-green-50 p-4 rounded-md">
                ...
                <p className="text-3xl font-bold text-green-700">${optimizationResults.totalSavings.toLocaleString()}</p>
              ...
              <div className="bg-blue-50 p-4 rounded-md">
                ...
                <p className="text-sm text-gray-600 mt-1">{Math.round((optimizationResults.durationSavings / projectData.duration) * 100)}% reduction</p>
              </div>
              
              <div className="bg-purple-50 p-4 rounded-md">
                <h3 className="font-semibold text-lg mb-2">Carbon Reduction</h3>
                <p className="text-3xl font-bold text-purple-700">{optimizationResults.carbonFootprint.savings.toLocaleString()} kg</p>
                <p className="text-sm text-gray-600 mt-1">{optimizationResults.carbonFootprint.savingsPercentage}% reduction</p>
              </div>
            </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Original Qty</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Optimized Qty</th>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Optimized Cost</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {optimizationResults.materials.map((material) => (
                      <tr key={material.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">{projectData.materials.find(m => m.id === material.id).quantity} {material.unit}</td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">${projectData.materials.find(m => m.id === material.id).totalCost.toLocaleString()}</td>
                        <td className="px-6 py-4 whitespace-nowrap">${material.totalCost.toLocaleString()}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-green-600">${material.savings.toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {optimizationResults.equipment.map((equipment) => (
                      <tr key={equipment.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">{projectData.equipment.find(e => e.id === equipment.id).quantity}</td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">${projectData.equipment.find(e => e.id === equipment.id).totalCost.toLocaleString()}</td>
                        <td className="px-6 py-4 whitespace-nowrap">${equipment.totalCost.toLocaleString()}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-green-600">${equipment.savings.toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {optimizationResults.labor.map((labor) => (
                      <tr key={labor.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">{projectData.labor.find(l => l.id === labor.id).quantity}</td>
                        <td className="px-6 py-4 whitespace-nowrap">{labor.quantity}</td>
                        <td className="px-6 py-4 whitespace-nowrap">${labor.hourlyRate}</td>
                        <td className="px-6 py-4 whitespace-nowrap">${projectData.labor.find(l => l.id === labor.id).totalCost.toLocaleString()}</td>
                        <td className="px-6 py-4 whitespace-nowrap">${labor.totalCost.toLocaleString()}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-green-600">${labor.savings.toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          <div className="mt-6">
            <p className="text-gray-600 mb-4">Click the "Optimize Resources" button above to generate an optimized resource allocation plan that can help reduce costs, time, and carbon footprint.</p>
